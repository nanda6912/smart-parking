@echo off
echo Force Vercel Deployment - Aggressive Method
echo.

echo Step 1: Update package.json version to force rebuild...
powershell -Command "(Get-Content package.json) -replace '\"version\": \"0.1.0\"', '\"version\": \"0.1.1\"' | Set-Content package.json"
